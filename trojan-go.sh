...
start_trojan-go() {
    if [[ -f '/etc/systemd/system/trojan-go.service' ]]; then
        if [[ -z "$TROJAN-GO_CUSTOMIZE" ]]; then
            systemctl start trojan-go
        else
            systemctl start "$TROJAN-GO_CUSTOMIZE"
        fi
    fi
    if [[ "$?" -ne 0 ]]; then
...
stop_trojan-go() {
    TROJAN-GO_CUSTOMIZE="$(systemctl list-units | grep 'trojan-go@' | awk -F ' ' '{print $1}')"
    if [[ -z "$TROJAN-GO_CUSTOMIZE" ]]; then
        systemctl stop trojan-go
    else
        systemctl stop "$TROJAN-GO_CUSTOMIZE"
    fi
    if [[ "$?" -ne '0' ]]; then
...
main() {
    ...
    echo "You may need to execute a command to remove dependent software: $PACKAGE_MANAGEMENT_REMOVE curl unzip"
    if [[ "$TROJAN-GO_RUNNING" -eq '1' ]]; then
        start_trojan-go
    else
